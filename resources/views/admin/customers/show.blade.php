<x-admin.layouts.app :title="$customer->name">
        ...
        <div class="flex items-center gap-2">
            ...
            @permission('customers.edit')
                <a href="{{ route('admin.customers.edit', $customer) }}">
                    <x-admin.button type="button" variant="secondary">Edit Profile</x-admin.button>
                </a>
            @endpermission
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <x-admin.stat-card label="Bookings" :value="$customer->bookings()->count()">
            <svg class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
        </x-admin.stat-card>

        <x-admin.stat-card label="Wallet Balance" :value="currency($customer->wallet_balance)">
            <svg class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
        ...
        <x-admin.stat-card label="Loyalty Points" :value="number_format($customer->loyalty_points)">
            <svg class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.562.562 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.562.562 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" />
            </svg>
        </x-admin.stat-card>

        <x-admin.stat-card label="Reviews" :value="$customer->reviews->count()">
            <svg class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
        ...
        <div class="space-y-6 lg:col-span-2">
            ...
            <div class="overflow-hidden rounded-2xl border border-luxury-border bg-luxury-charcoal">
                <div class="border-b border-luxury-border px-6 py-4">
                    <h3 class="text-sm font-semibold text-luxury-white">Booking History</h3>
                </div>
                <div class="overflow-x-auto">
                    <table class="w-full text-start text-sm">
                        ...
                        <tbody class="divide-y divide-luxury-border/60">
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-6 py-8 text-center text-luxury-muted">No bookings yet.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            <div class="overflow-hidden rounded-2xl border border-luxury-border bg-luxury-charcoal">
                <div class="border-b border-luxury-border px-6 py-4">
                    <h3 class="text-sm font-semibold text-luxury-white">Reviews</h3>
                </div>
                    ...
                    @forelse ($customer->reviews as $review)
                        <div class="px-6 py-4">
                            ...
                            <p class="mt-1 text-xs text-luxury-muted">
                                {{ $review->driver?->name ?? 'Unknown driver' }} &middot; {{ $review->vehicle?->name ?? 'Unknown vehicle' }} &middot; {{ $review->created_at->format('M d, Y') }}
                            </p>
                        </div>
                    @empty
        ...
        <div class="space-y-6">
            ...
            <div class="rounded-2xl border border-luxury-border bg-luxury-charcoal p-6" x-data="{ open: false }">
                <div class="mb-4 flex items-center justify-between">
                    <h3 class="text-sm font-semibold text-luxury-white">Saved Addresses</h3>
                    @permission('customers.edit')
                ...
                <div x-show="open" x-cloak x-transition class="mb-4">
                    <form method="POST" action="{{ route('admin.customers.addresses.store', $customer) }}" class="space-y-3">
                        @csrf
                        <input type="text" name="label" placeholder="Label (e.g. Home)" required
                        ...
                        <x-admin.button type="submit" variant="primary" class="w-full !py-2 text-xs">Save Address</x-admin.button>
                    </form>
                </div>
                        ...
                        <div class="rounded-lg border border-luxury-border/60 p-3">
                            <div class="flex items-center justify-between">
                                ...
                                @if ($address->is_default)
                                    <span class="rounded-full bg-luxury-gold/10 px-2 py-0.5 text-[10px] text-luxury-gold">Default</span>
                                @endif
                            </div>
                                ...
                                <div class="mt-2 flex items-center gap-2">
                                    @unless ($address->is_default)
                                        <form method="POST" action="{{ route('admin.addresses.default', $address) }}">
                                            @csrf
                                            <button type="submit" class="text-[11px] text-luxury-muted hover:text-luxury-gold">Make default</button>
                                        </form>
                                    @endunless
                                    <form method="POST" action="{{ route('admin.addresses.destroy', $address) }}" onsubmit="return confirm('Remove this address?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-[11px] text-red-400 hover:text-red-300">Remove</button>
                                    </form>
                                </div>
                            @endpermission
                        </div>
                    @empty
                        <p class="text-xs text-luxury-muted">No saved addresses.</p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-luxury-border bg-luxury-charcoal p-6" x-data="{ open: false }">
                <div class="mb-4 flex items-center justify-between">
                    <div>
                        <h3 class="text-sm font-semibold text-luxury-white">Wallet</h3>
                ...
                <div x-show="open" x-cloak x-transition class="mb-4">
                    <form method="POST" action="{{ route('admin.customers.wallet.store', $customer) }}" class="space-y-3">
                        ...
                        <input type="text" name="reason" placeholder="Reason (optional)"
                            class="w-full rounded-lg border border-luxury-border bg-luxury-graphite px-3 py-2 text-sm text-luxury-white placeholder:text-luxury-muted focus:border-luxury-gold focus:outline-none focus:ring-1 focus:ring-luxury-gold">
                        <x-admin.button type="submit" variant="primary" class="w-full !py-2 text-xs">Update Wallet</x-admin.button>
                    </form>
                ...
                <div class="scrollbar-luxury max-h-56 space-y-2 overflow-y-auto">
                    ...
                    @empty
                        <p class="text-xs text-luxury-muted">No wallet activity yet.</p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-luxury-border bg-luxury-charcoal p-6" x-data="{ open: false }">
                <div class="mb-4 flex items-center justify-between">
                    <div>
                        <h3 class="text-sm font-semibold text-luxury-white">Loyalty Points</h3>
                ...
                <div x-show="open" x-cloak x-transition class="mb-4">
                    <form method="POST" action="{{ route('admin.customers.loyalty.store', $customer) }}" class="space-y-3">
                        @csrf
                        <select name="type" class="w-full rounded-lg border border-luxury-border bg-luxury-graphite px-3 py-2 text-sm text-luxury-white focus:border-luxury-gold focus:outline-none focus:ring-1 focus:ring-luxury-gold">
                            <option value="earned">Earn (add points)</option>
                            <option value="redeemed">Redeem (deduct points)</option>
                        </select>
                        <input type="number" name="points" min="1" placeholder="Points" required
                            class="w-full rounded-lg border border-luxury-border bg-luxury-graphite px-3 py-2 text-sm text-luxury-white placeholder:text-luxury-muted focus:border-luxury-gold focus:outline-none focus:ring-1 focus:ring-luxury-gold">
                        <input type="text" name="reason" placeholder="Reason (optional)"
                            class="w-full rounded-lg border border-luxury-border bg-luxury-graphite px-3 py-2 text-sm text-luxury-white placeholder:text-luxury-muted focus:border-luxury-gold focus:outline-none focus:ring-1 focus:ring-luxury-gold">
                        <x-admin.button type="submit" variant="primary" class="w-full !py-2 text-xs">Update Points</x-admin.button>
                    </form>
                ...
                <div class="scrollbar-luxury max-h-56 space-y-2 overflow-y-auto">
                    ...
                    @empty
                        <p class="text-xs text-luxury-muted">No loyalty activity yet.</p>
                    @endforelse
                </div>
            </div>
        </div>
