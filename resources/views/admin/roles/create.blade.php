<x-admin.layouts.app :title="__('Create Role')">
    <div class="mb-6">
        <h2 class="text-2xl font-semibold text-luxury-white">{{ __('Create Role') }}</h2>
        <p class="mt-1 text-sm text-luxury-muted">{{ __('Define a role and assign its module permissions.') }}</p>
    </div>
    ...
    <form method="POST" action="{{ route('admin.roles.store') }}" class="space-y-6">
        ...
        <div class="flex items-center gap-3">
            <x-admin.button type="submit" variant="primary">{{ __('Create Role') }}</x-admin.button>
