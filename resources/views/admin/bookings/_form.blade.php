...
<div class="space-y-6"
    ...
    <div class="space-y-5 rounded-2xl border border-luxury-border bg-luxury-charcoal p-6">
        <h3 class="text-sm font-semibold text-luxury-white">Booking Details</h3>
        ...
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div>
                <x-admin.input-label for="vehicle_id" value="Vehicle" />
            ...
            <div>
                ...
                <select id="driver_id" name="driver_id"
                    ...
                    @foreach ($drivers as $d)
                        <option value="{{ $d->id }}" @selected(old('driver_id', $booking?->driver_id) == $d->id)>{{ $d->name }} {{ $d->is_online ? '(online)' : '' }}</option>
                    @endforeach
                </select>
            ...
            <div>
                <x-admin.input-label for="type" value="Booking Type" />
    ...
    {{-- Trip Details --}}
    <div class="space-y-5 rounded-2xl border border-luxury-border bg-luxury-charcoal p-6">
        <h3 class="text-sm font-semibold text-luxury-white">Trip Details</h3>

        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
            <div>
                <x-admin.input-label for="pickup_location" value="Pickup" />
            ...
            <div>
                <x-admin.input-label for="dropoff_location" value="Dropoff" />
        ...
        <div>
            <div class="mb-2 flex items-center justify-between">
                <x-admin.input-label value="Stops (optional)" class="!mb-0" />
                <button type="button" @click="stops.push('')" class="text-xs font-medium text-luxury-gold hover:text-luxury-gold-light">+ Add Stop</button>
            </div>
            <div class="space-y-2">
                <template x-for="(stop, index) in stops" :key="index">
                    <div class="flex items-center gap-2">
                        <input type="text" :name="'stops[' + index + ']'" x-model="stops[index]" placeholder="Stop address"
        ...
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
            <div>
                <x-admin.input-label for="pickup_date" value="Pickup Date" />
            ...
            <div>
                <x-admin.input-label for="pickup_time" value="Pickup Time" />
            ...
            <div x-show="type === 'round_trip'" x-cloak>
                <x-admin.input-label for="return_date" value="Return Date" />
            ...
            <div x-show="type === 'round_trip'" x-cloak>
                <x-admin.input-label for="return_time" value="Return Time" />
            ...
            <div x-show="type === 'hourly'" x-cloak>
                <x-admin.input-label for="hours" value="Duration (hours)" />
            ...
            <div x-show="type !== 'hourly'" x-cloak>
                <x-admin.input-label for="distance_km" value="Distance (km)" />
    ...
    {{-- Passenger Details --}}
    <div class="space-y-5 rounded-2xl border border-luxury-border bg-luxury-charcoal p-6">
        <h3 class="text-sm font-semibold text-luxury-white">Passenger Details</h3>
        ...
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
            ...
            <div>
                <x-admin.input-label for="waiting_minutes" value="Waiting Time (minutes)" />
            ...
            <div class="flex items-end pb-1">
                <label class="flex items-center gap-2 text-sm text-luxury-muted">
                    ...
                    Trip includes toll roads
                </label>
            </div>

            <div class="sm:col-span-2">
                <x-admin.input-label for="notes" value="Notes" />
                <textarea id="notes" name="notes" rows="3" placeholder="Special instructions, flight number, etc."
    ...
    <div class="space-y-5 rounded-2xl border border-luxury-border bg-luxury-charcoal p-6">
        ...
        <template x-if="activeRule()">
            <div class="rounded-xl border border-luxury-border/60 bg-luxury-graphite/40 p-4">
                <p class="mb-3 text-xs font-medium uppercase tracking-wider text-luxury-muted">
                    Fare Breakdown <span class="text-luxury-gold" x-text="'(' + activeRule().label + ')'"></span>
                </p>
                <dl class="space-y-1.5 text-sm">
                    <div class="flex items-center justify-between" x-show="breakdown().base_fare > 0">
                        <dt class="text-luxury-muted">Base Fare</dt>
                    ...
                    <div class="flex items-center justify-between" x-show="breakdown().distance_fare > 0">
                        <dt class="text-luxury-muted">KM Fare</dt>
                    ...
                    <div class="flex items-center justify-between" x-show="breakdown().hour_fare > 0">
                        <dt class="text-luxury-muted">Hour Fare</dt>
                    ...
                    <div class="flex items-center justify-between" x-show="breakdown().waiting_charge > 0">
                        <dt class="text-luxury-muted">Waiting Charges</dt>
                    ...
                    <div class="flex items-center justify-between" x-show="breakdown().night_charge > 0">
                        <dt class="text-luxury-muted">Night Charges</dt>
                    ...
                    <div class="flex items-center justify-between" x-show="breakdown().weekend_charge > 0">
                        <dt class="text-luxury-muted">Weekend Charges</dt>
                    ...
                    <div class="flex items-center justify-between" x-show="breakdown().toll_charge > 0">
                        <dt class="text-luxury-muted">Toll Charges</dt>
                    ...
                    <div class="flex items-center justify-between" x-show="breakdown().airport_surcharge > 0">
                        <dt class="text-luxury-muted">Airport Surcharge</dt>
        ...
        <div>
            <x-admin.input-label for="fare_amount" value="Fare Amount" />
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center">
                ...
                <template x-if="activeRule()">
                    <button type="button" @click="document.getElementById('fare_amount').value = breakdown().total"
                        ...
                        Use suggested total &mdash; <span class="font-semibold text-luxury-gold underline" x-text="money(breakdown().total)"></span>
                    </button>
                </template>
            </div>
            <p class="mt-2 text-xs text-luxury-muted">Auto-estimated by the dynamic pricing engine from the vehicle's category pricing. You can override it.</p>
