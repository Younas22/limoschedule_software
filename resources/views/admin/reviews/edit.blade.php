<x-admin.layouts.app :title="__('Edit Review')">
    <div class="mb-6">
        <h2 class="text-2xl font-semibold text-luxury-white">{{ __('Edit Review') }}</h2>
        <p class="mt-1 text-sm text-luxury-muted">{{ __('Update the review from :name.', ['name' => $review->customer?->name ?? __('this customer')]) }}</p>
    </div>

    <form method="POST" action="{{ route('admin.reviews.update', $review) }}" class="space-y-6">
        @csrf
        @method('PUT')
        @include('admin.reviews._form')

        <div class="flex items-center gap-3">
            <x-admin.button type="submit" variant="primary">{{ __('Save Changes') }}</x-admin.button>
            <a href="{{ route('admin.reviews.index') }}" class="text-sm text-luxury-muted hover:text-luxury-white">{{ __('Cancel') }}</a>
        </div>
    </form>
</x-admin.layouts.app>
