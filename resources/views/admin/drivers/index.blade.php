<x-admin.layouts.app :title="'Drivers'">
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-2xl font-semibold text-luxury-white">Drivers</h2>
            <p class="mt-1 text-sm text-luxury-muted">Manage your chauffeurs — documents, vehicle assignment, and commission.</p>
        </div>
                ...
                <div class="overflow-hidden rounded-2xl border border-luxury-border bg-luxury-charcoal transition hover:border-luxury-gold/40">
                    <div class="flex items-start gap-4 p-5">
                        <div class="relative shrink-0">
                            ...
                            <span class="absolute -end-0.5 -top-0.5 h-3.5 w-3.5 rounded-full border-2 border-luxury-charcoal {{ $driver->is_online ? 'bg-emerald-400' : 'bg-luxury-slate' }}" title="{{ $driver->is_online ? 'Online' : 'Offline' }}"></span>
                        </div>
                    ...
                    <div class="border-t border-luxury-border px-5 py-4">
                        <div class="flex items-center justify-between text-xs">
                            <span class="text-luxury-muted">Assigned Vehicle</span>
                            <span class="font-medium text-luxury-white">{{ $driver->vehicle?->name ?? 'Unassigned' }}</span>
                        </div>
                        <div class="mt-2 flex items-center justify-between text-xs">
                            <span class="text-luxury-muted">Commission</span>
                            <span class="font-medium text-luxury-gold">{{ number_format($driver->commission_rate, 2) }}%</span>
                        </div>
                        <div class="mt-2 flex items-center justify-between text-xs">
                            <span class="text-luxury-muted">Rating</span>
                            ...
                            @else
                                <span class="text-luxury-muted">No reviews yet</span>
                            @endif
                        </div>
                    </div>
