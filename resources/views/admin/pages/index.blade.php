<x-admin.layouts.app :title="'Pages'">
    <div class="mb-6">
        <h2 class="text-2xl font-semibold text-luxury-white">Pages</h2>
        <p class="mt-1 text-sm text-luxury-muted">Manage the content sections that make up your public website.</p>
    </div>
    ...
    <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
        @foreach ($pages as $page)
            <a href="{{ route('admin.pages.edit', $page) }}" class="group rounded-2xl border border-luxury-border bg-luxury-charcoal p-6 transition hover:border-luxury-gold/40">
                ...
                <p class="mt-4 text-sm text-luxury-muted">{{ $page->sections_count }} {{ \Illuminate\Support\Str::plural('section', $page->sections_count) }}</p>
            </a>
        @endforeach
    </div>
</x-admin.layouts.app>
