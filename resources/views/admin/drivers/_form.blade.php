...
<div class="space-y-6">
    ...
    <div class="rounded-2xl border border-luxury-border bg-luxury-charcoal p-6">
        <div x-data="{ preview: '{{ $driver?->photo_url }}' }">
            <x-admin.input-label value="Driver Photo" />
    ...
    <div class="space-y-5 rounded-2xl border border-luxury-border bg-luxury-charcoal p-6">
        <h3 class="text-sm font-semibold text-luxury-white">Personal Information</h3>
    ...
    {{-- Documents --}}
    <div class="space-y-5 rounded-2xl border border-luxury-border bg-luxury-charcoal p-6">
        <h3 class="text-sm font-semibold text-luxury-white">Documents</h3>

        <div class="grid grid-cols-1 gap-5 sm:grid-cols-3">
            <div>
                <x-admin.input-label for="license_number" value="License Number" />
            ...
            <div>
                <x-admin.input-label for="passport_number" value="Passport Number" />
            ...
            <div>
                <x-admin.input-label for="national_id" value="National ID" />
    ...
    <div class="space-y-5 rounded-2xl border border-luxury-border bg-luxury-charcoal p-6">
        <h3 class="text-sm font-semibold text-luxury-white">Assignment &amp; Commission</h3>

        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
            <div>
                <x-admin.input-label for="vehicle_id" value="Assigned Vehicle" />
                <select id="vehicle_id" name="vehicle_id"
                    ...
                    @foreach ($vehicles as $v)
                        <option value="{{ $v->id }}" @selected(old('vehicle_id', $driver?->vehicle_id) == $v->id)>{{ $v->name }} ({{ $v->plate_number ?? 'no plate' }})</option>
                    @endforeach
                </select>
            ...
            <div>
                <x-admin.input-label for="commission_rate" value="Commission Rate (%)" />
