<x-admin.layouts.app :title="'Bookings Report'">
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-2xl font-semibold text-luxury-white">Bookings Report</h2>
            <p class="mt-1 text-sm text-luxury-muted">All bookings within the selected date range.</p>
        </div>
    ...
    <div class="mb-6 flex flex-col gap-4 rounded-2xl border border-luxury-border bg-luxury-charcoal p-5 sm:flex-row sm:items-end sm:justify-between">
        <form method="GET">
            <x-admin.reports.date-filter :from="$from" :to="$to">
                <div>
                    <label class="mb-1 block text-xs text-luxury-muted">Status</label>
                ...
                <div>
                    <label class="mb-1 block text-xs text-luxury-muted">Type</label>
    ...
    <div class="mb-6 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-5">
        <x-admin.stat-card label="Total" :value="number_format($summary['total'])">
        ...
        @foreach (['completed' => 'Completed', 'cancelled' => 'Cancelled', 'pending' => 'Pending', 'confirmed' => 'Confirmed'] as $key => $label)
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-start text-sm">
                ...
                <tbody class="divide-y divide-luxury-border/60">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-10 text-center text-luxury-muted">No bookings found in this range.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
