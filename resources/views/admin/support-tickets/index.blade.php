<x-admin.layouts.app :title="'Support Tickets'">
    <div class="mb-6">
        <h2 class="text-2xl font-semibold text-luxury-white">Support Tickets</h2>
        <p class="mt-1 text-sm text-luxury-muted">Customer support tickets raised from the dashboard.</p>
    </div>
    ...
    <form method="GET" class="mb-6 flex flex-wrap items-center gap-3">
        <div class="relative flex-1 sm:max-w-sm">
            ...
            <input type="search" name="search" value="{{ request('search') }}" placeholder="Search by ticket #, subject, or customer..."
        ...
        @if (request()->filled('status') || request()->filled('search'))
            <a href="{{ route('admin.support-tickets.index') }}" class="text-xs text-luxury-muted hover:text-luxury-gold">Clear filters</a>
        @endif
    </form>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-start text-sm">
                ...
                <tbody class="divide-y divide-luxury-border/60">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-10 text-center text-luxury-muted">No support tickets found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    @if ($tickets->hasPages())
        <div class="mt-6 flex items-center justify-between text-sm text-luxury-muted">
            <div>
                ...
                @else
                    <a href="{{ $tickets->previousPageUrl() }}" class="rounded-lg border border-luxury-border px-3 py-1.5 hover:border-luxury-gold/40 hover:text-luxury-gold">Previous</a>
                @endif
            </div>
            <p>Page {{ $tickets->currentPage() }} of {{ $tickets->lastPage() }}</p>
            <div>
                @if ($tickets->hasMorePages())
                    <a href="{{ $tickets->nextPageUrl() }}" class="rounded-lg border border-luxury-border px-3 py-1.5 hover:border-luxury-gold/40 hover:text-luxury-gold">Next</a>
                @else
