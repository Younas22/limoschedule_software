...
<x-admin.layouts.app :title="'Notification Preferences'">
    <div class="mb-6">
        <h2 class="text-2xl font-semibold text-luxury-white">Notification Preferences</h2>
        <p class="mt-1 text-sm text-luxury-muted">Choose which channels deliver each booking event. SMS and push are wired into the architecture but not yet active.</p>
    </div>
    ...
    <form method="POST" action="{{ route('admin.notification-settings.update') }}">
                            ...
                            @foreach ($channels as $field => $label)
                                <th class="px-6 py-3 text-center font-medium">
                                    ...
                                    @if (in_array($field, $futureChannels, true))
                                        <span class="ms-1 rounded-full bg-luxury-slate px-1.5 py-0.5 text-[9px] normal-case text-luxury-muted">Soon</span>
                                    @endif
                                </th>
                            @endforeach
                        </tr>
        ...
        <div class="mt-6 flex items-center justify-end gap-3">
            ...
            <x-admin.button type="submit" variant="primary">Save Preferences</x-admin.button>
        </div>
    </form>
</x-admin.layouts.app>
