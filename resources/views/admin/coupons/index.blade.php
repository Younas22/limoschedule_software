<x-admin.layouts.app :title="'Coupons'">
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-2xl font-semibold text-luxury-white">Coupons</h2>
            <p class="mt-1 text-sm text-luxury-muted">Discount codes shown to customers on their dashboard.</p>
        </div>
                ...
                <tbody class="divide-y divide-luxury-border/60">
                    @forelse ($coupons as $coupon)
                        <tr class="hover:bg-luxury-graphite">
                            ...
                            <td class="px-6 py-3 text-luxury-muted">{{ $coupon->expires_at?->format('M d, Y') ?? 'Never' }}</td>
                            <td class="px-6 py-3">
                                @if (! $coupon->is_active)
                                ...
                                @elseif ($coupon->isExpired())
                                    <span class="rounded-full bg-red-500/10 px-2.5 py-1 text-xs font-medium text-red-400">Expired</span>
                                @elseif ($coupon->isExhausted())
                                    <span class="rounded-full bg-red-500/10 px-2.5 py-1 text-xs font-medium text-red-400">Exhausted</span>
                                @else
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-10 text-center text-luxury-muted">No coupons created yet.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    @if ($coupons->hasPages())
        <div class="mt-6 flex items-center justify-between text-sm text-luxury-muted">
            <div>
                ...
                @else
                    <a href="{{ $coupons->previousPageUrl() }}" class="rounded-lg border border-luxury-border px-3 py-1.5 hover:border-luxury-gold/40 hover:text-luxury-gold">Previous</a>
                @endif
            </div>
            <p>Page {{ $coupons->currentPage() }} of {{ $coupons->lastPage() }}</p>
            <div>
                @if ($coupons->hasMorePages())
                    <a href="{{ $coupons->nextPageUrl() }}" class="rounded-lg border border-luxury-border px-3 py-1.5 hover:border-luxury-gold/40 hover:text-luxury-gold">Next</a>
                @else
