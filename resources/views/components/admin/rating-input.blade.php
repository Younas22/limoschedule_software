@props(['name' => 'rating', 'value' => 0])

@php
    $starPath = 'M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.958a1 1 0 00.95.69h4.162c.969 0 1.371 1.24.588 1.81l-3.368 2.447a1 1 0 00-.363 1.118l1.287 3.957c.3.922-.755 1.688-1.538 1.118l-3.367-2.446a1 1 0 00-1.176 0l-3.367 2.446c-.783.57-1.838-.196-1.538-1.118l1.287-3.957a1 1 0 00-.363-1.118L2.062 9.385c-.783-.57-.38-1.81.588-1.81h4.163a1 1 0 00.95-.69l1.286-3.958z';
@endphp

<div x-data="{ rating: {{ (int) $value ?: 5 }} }" class="flex items-center gap-1">
    @for ($i = 1; $i <= 5; $i++)
        <button type="button" @click="rating = {{ $i }}" class="p-0.5 transition hover:scale-110">
            <svg class="h-6 w-6" :class="rating >= {{ $i }} ? 'text-luxury-gold' : 'text-luxury-border'" fill="currentColor" viewBox="0 0 20 20">
                <path d="{{ $starPath }}" />
            </svg>
        </button>
    @endfor
    <input type="hidden" name="{{ $name }}" x-model="rating">
    <span class="ms-2 text-sm text-luxury-muted" x-text="rating + '{{ __(' / 5') }}'"></span>
</div>
