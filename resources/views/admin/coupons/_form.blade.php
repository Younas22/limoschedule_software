...
<div class="space-y-5 rounded-2xl border border-luxury-border bg-luxury-charcoal p-6">
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
        <div>
            <x-admin.input-label for="code" value="Coupon Code" />
            <x-admin.text-input id="code" name="code" type="text" value="{{ old('code', $coupon?->code) }}" placeholder="e.g. WELCOME10" required autofocus class="uppercase" />
            <x-admin.input-error :messages="$errors->get('code')" />
        </div>

        <div>
            <x-admin.input-label for="description" value="Description" />
            <x-admin.text-input id="description" name="description" type="text" value="{{ old('description', $coupon?->description) }}" placeholder="e.g. 10% off your first ride" />
    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
        <div>
            <x-admin.input-label for="discount_type" value="Discount Type" />
        ...
        <div>
            <x-admin.input-label for="discount_value" value="Discount Value" />
        ...
        <div>
            <x-admin.input-label for="max_discount" value="Max Discount (optional)" />
            <x-admin.text-input id="max_discount" name="max_discount" type="number" step="0.01" min="0" value="{{ old('max_discount', $coupon?->max_discount) }}" placeholder="For percentage coupons" />
    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
        <div>
            <x-admin.input-label for="min_fare" value="Minimum Fare (optional)" />
        ...
        <div>
            <x-admin.input-label for="usage_limit" value="Usage Limit (optional)" />
            <x-admin.text-input id="usage_limit" name="usage_limit" type="number" min="1" value="{{ old('usage_limit', $coupon?->usage_limit) }}" placeholder="Unlimited" />
            <x-admin.input-error :messages="$errors->get('usage_limit')" />
        </div>

        <div>
            <x-admin.input-label for="expires_at" value="Expires At (optional)" />
