<x-admin.layouts.app :title="'Tags'">
    <div class="mb-6">
        <h2 class="text-2xl font-semibold text-luxury-white">Tags</h2>
        <p class="mt-1 text-sm text-luxury-muted">Manage tags used across blog posts.</p>
    </div>
    ...
    @permission('blog.create')
        <form method="POST" action="{{ route('admin.blog.tags.store') }}" class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start">
            @csrf
            <div class="flex-1">
                <x-admin.text-input name="name" type="text" placeholder="New tag name" required class="sm:max-w-xs" />
                <x-admin.input-error :messages="$errors->get('name')" />
            </div>
            <x-admin.button type="submit" variant="primary">Add Tag</x-admin.button>
        </form>
    @endpermission
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-start text-sm">
                ...
                <tbody class="divide-y divide-luxury-border/60">
                        ...
                        <tr class="hover:bg-luxury-graphite" x-data="{ editing: false }">
                            <td class="px-6 py-3">
                                ...
                                <form x-show="editing" x-cloak method="POST" action="{{ route('admin.blog.tags.update', $tag) }}" class="flex items-center gap-2">
                                    ...
                                    <button type="submit" class="text-xs text-luxury-gold hover:text-luxury-gold-light">Save</button>
                                    <button type="button" @click="editing = false" class="text-xs text-luxury-muted hover:text-luxury-white">Cancel</button>
                                </form>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-10 text-center text-luxury-muted">No tags yet.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
