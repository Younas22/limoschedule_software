...
<div class="space-y-5 rounded-2xl border border-luxury-border bg-luxury-charcoal p-6">
    <div>
        <x-admin.input-label for="name" value="Category Name" />
        <x-admin.text-input id="name" name="name" type="text" value="{{ old('name', $category?->name) }}" placeholder="e.g. Sedan" required autofocus />
        <x-admin.input-error :messages="$errors->get('name')" />
    </div>

    <div>
        <x-admin.input-label for="description" value="Description" />
        <textarea id="description" name="description" rows="3" placeholder="Optional short description shown to customers"
    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
        <div x-data="{ preview: '{{ $category?->icon_url }}' }">
            <x-admin.input-label value="Icon" />
            <div class="flex items-center gap-4">
                <div class="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-full border border-luxury-border bg-luxury-graphite">
                    ...
                    <template x-if="!preview">
                        <span class="text-xs text-luxury-muted">No icon</span>
                    </template>
                </div>
            ...
            <p class="mt-2 text-xs text-luxury-muted">Small square icon for lists. Max 512KB.</p>
            <x-admin.input-error :messages="$errors->get('icon')" />
        </div>

        <div x-data="{ preview: '{{ $category?->image_url }}' }">
            <x-admin.input-label value="Image" />
            <div class="flex items-center gap-4">
            ...
            <p class="mt-2 text-xs text-luxury-muted">Showcase photo for this category. Max 2MB.</p>
