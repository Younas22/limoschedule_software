<x-admin.layouts.app :title="__('Cities')">
    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        @permission('locations.create')
            <a href="{{ route('admin.locations.cities.create') }}">
                ...
                <tbody class="divide-y divide-luxury-border/60">
                    @forelse ($items as $city)
                        <tr class="hover:bg-luxury-graphite">
                            <td class="px-6 py-3 font-medium text-luxury-white">{{ $city->name }}</td>
                            <td class="px-6 py-3 text-luxury-muted">{{ $city->state->name }}</td>
                            <td class="px-6 py-3 text-luxury-muted">{{ $city->country->name }}</td>
                            <td class="px-6 py-3"><x-admin.status-badge :active="$city->is_active" /></td>
                            <td class="px-6 py-3">
