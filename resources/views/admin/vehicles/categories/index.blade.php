<x-admin.layouts.app :title="__('Fleet Categories')">
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-2xl font-semibold text-luxury-white">{{ __('Fleet Categories') }}</h2>
            <p class="mt-1 text-sm text-luxury-muted">{{ __('Organize your fleet into categories such as Sedan, SUV, or Limousine.') }}</p>
        </div>
            ...
            <table class="w-full text-start text-sm">
                <thead>
                    <tr class="border-b border-luxury-border text-xs uppercase tracking-wider text-luxury-muted">
                        ...
                        <th class="px-6 py-3 font-medium">{{ __('Icon') }}</th>
                ...
                <tbody class="divide-y divide-luxury-border/60">
                            ...
                            <td class="px-6 py-3">
                                <div class="flex h-10 w-10 items-center justify-center overflow-hidden rounded-full border border-luxury-border bg-luxury-graphite">
                                    @if ($category->icon_url)
                                        <img src="{{ $category->icon_url }}" alt="{{ __(':name icon', ['name' => $category->name]) }}" class="h-full w-full object-cover">
                                    @else
                            ...
                            <td class="px-6 py-3">
                                <div class="flex flex-wrap items-center justify-end gap-2">
                                    ...
                                    @permission('vehicles.delete')
                                        <form method="POST" action="{{ route('admin.vehicles.categories.destroy', $category) }}" onsubmit="return confirm('{{ __('Delete this category?') }}');">
                                            @csrf
                                            @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-10 text-center text-luxury-muted">{{ __('No fleet categories added yet.') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
