...
<div class="space-y-6">
    ...
    <div class="space-y-5 rounded-2xl border border-luxury-border bg-luxury-charcoal p-6">
        <h3 class="text-sm font-semibold text-luxury-white">Basic Information</h3>

        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
            <div class="sm:col-span-2">
                <x-admin.input-label for="vehicle_category_id" value="Category" />
            ...
            <div>
                <x-admin.input-label for="name" value="Vehicle Name" />
                <x-admin.text-input id="name" name="name" type="text" value="{{ old('name', $vehicle?->name) }}" placeholder="e.g. Mercedes-Benz E-Class" required autofocus />
                <x-admin.input-error :messages="$errors->get('name')" />
            </div>

            <div>
                <x-admin.input-label for="plate_number" value="Plate Number" />
                <x-admin.text-input id="plate_number" name="plate_number" type="text" value="{{ old('plate_number', $vehicle?->plate_number) }}" placeholder="Optional" />
                <x-admin.input-error :messages="$errors->get('plate_number')" />
            </div>

            <div>
                <x-admin.input-label for="brand" value="Brand" />
                <x-admin.text-input id="brand" name="brand" type="text" value="{{ old('brand', $vehicle?->brand) }}" placeholder="e.g. Mercedes-Benz" required />
                <x-admin.input-error :messages="$errors->get('brand')" />
            </div>

            <div>
                <x-admin.input-label for="model" value="Model" />
                <x-admin.text-input id="model" name="model" type="text" value="{{ old('model', $vehicle?->model) }}" placeholder="e.g. E-Class" required />
                <x-admin.input-error :messages="$errors->get('model')" />
            </div>

            <div>
                <x-admin.input-label for="year" value="Year" />
            ...
            <div>
                <x-admin.input-label for="seats" value="Seats" />
            ...
            <div>
                <x-admin.input-label for="luggage" value="Luggage" />
            ...
            <div>
                <x-admin.input-label for="transmission" value="Transmission" />
                <select id="transmission" name="transmission" required
                    class="w-full rounded-lg border border-luxury-border bg-luxury-charcoal px-4 py-3 text-sm text-luxury-white focus:border-luxury-gold focus:outline-none focus:ring-1 focus:ring-luxury-gold transition">
                    <option value="automatic" @selected(old('transmission', $vehicle?->transmission ?? 'automatic') === 'automatic')>Automatic</option>
            ...
            <div>
                <x-admin.input-label for="fuel_type" value="Fuel Type" />
                <select id="fuel_type" name="fuel_type" required
                    class="w-full rounded-lg border border-luxury-border bg-luxury-charcoal px-4 py-3 text-sm text-luxury-white focus:border-luxury-gold focus:outline-none focus:ring-1 focus:ring-luxury-gold transition">
                    @foreach (['petrol' => 'Petrol', 'diesel' => 'Diesel', 'hybrid' => 'Hybrid', 'electric' => 'Electric'] as $value => $label)
                        <option value="{{ $value }}" @selected(old('fuel_type', $vehicle?->fuel_type ?? 'petrol') === $value)>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="sm:col-span-2">
                ...
                <textarea id="description" name="description" rows="3" placeholder="Optional description shown to customers"
    ...
    <div class="space-y-5 rounded-2xl border border-luxury-border bg-luxury-charcoal p-6">
        <div>
            <h3 class="text-sm font-semibold text-luxury-white">Pricing</h3>
            <p class="text-xs text-luxury-muted">Stored in the default currency ({{ \App\Models\Currency::default()?->code ?? 'USD' }}) and converted automatically for display.</p>
        </div>
        ...
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div>
                <x-admin.input-label for="price_per_km" value="Price per KM" />
            ...
            <div>
                <x-admin.input-label for="price_per_hour" value="Price per Hour" />
            ...
            <div>
                <x-admin.input-label for="airport_price" value="Airport Price" />
            ...
            <div>
                <x-admin.input-label for="night_charges" value="Night Charges" />
    ...
    <div class="space-y-4 rounded-2xl border border-luxury-border bg-luxury-charcoal p-6">
        <h3 class="text-sm font-semibold text-luxury-white">Features</h3>
    ...
    {{-- Images --}}
    <div class="space-y-5 rounded-2xl border border-luxury-border bg-luxury-charcoal p-6">
        <h3 class="text-sm font-semibold text-luxury-white">Images</h3>
        ...
        <div x-data="{ preview: '{{ $vehicle?->image_url }}' }">
            ...
            <div class="flex items-center gap-4">
                <div class="flex h-20 w-32 shrink-0 items-center justify-center overflow-hidden rounded-lg border border-luxury-border bg-luxury-graphite">
                    <template x-if="preview">
                        <img :src="preview" alt="Featured image preview" class="h-full w-full object-cover">
                    </template>
                    <template x-if="!preview">
            ...
            <p class="mt-2 text-xs text-luxury-muted">Shown on the vehicle card. Max 2MB.</p>
            <x-admin.input-error :messages="$errors->get('image')" />
        </div>

        @if ($vehicle && $vehicle->images->isNotEmpty())
            <div>
                <x-admin.input-label value="Gallery" />
                <div class="grid grid-cols-3 gap-3 sm:grid-cols-5">
                    @foreach ($vehicle->images as $galleryImage)
                        <div class="group relative aspect-square overflow-hidden rounded-lg border border-luxury-border bg-luxury-graphite">
                            <img src="{{ $galleryImage->image_url }}" alt="Gallery image" class="h-full w-full object-cover">
                            <form method="POST" action="{{ route('admin.vehicles.images.destroy', $galleryImage) }}" onsubmit="return confirm('Remove this image?');" class="absolute end-1 top-1">
                                @csrf
                                @method('DELETE')
        ...
        <div>
            <x-admin.input-label for="images" value="Add Gallery Photos" />
            ...
            <p class="mt-2 text-xs text-luxury-muted">You can select multiple photos. Max 2MB each.</p>
